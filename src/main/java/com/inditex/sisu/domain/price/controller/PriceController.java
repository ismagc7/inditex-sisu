...
public class PriceController extends CommonController {
    ...
    @GetMapping
    public JSONResult<Page<PriceDto>> getByFilter(@ModelAttribute PriceRequestFilterDto filter, Pageable pageable) {
        Page<PriceDto> priceList = priceService.getAllByFilter(filter, pageable);
        return new JSONResult<>(priceList);
    }
